...
function decryptMessage(encryptedMessage, iv, passKey) {
  try {
    ...
    const decipher = crypto.createDecipheriv('aes-256-ctr', Buffer.from(passKey, 'hex'), Buffer.from(iv, 'hex'));
...
io.on('connection', (socket) => {
  ...
  socket.on('dataFromEmitter', async (message) => {
    ...
    const decryptedMessage = decryptMessage(message.encryptedMessage, message.iv, message.passKey);
...
async function saveDecryptedMessageToDatabase(decryptedMessage) {
  try {
    ...
    console.log('Message saved to the time-series database for minute:', formattedTimestamp);
